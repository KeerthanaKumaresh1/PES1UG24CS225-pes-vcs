...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    unsigned char *obj = malloc(total);
    if (!obj) return -1;
    ...
    if (write(fd, obj, total) < 0) { free(obj); close(fd); return -1; }
    fsync(fd);
    close(fd);
    free(obj);
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    if (fread(buf, 1, total, f) != total) { free(buf); fclose(f); return -1; }
    fclose(f);
    ...
    unsigned char *null_pos = memchr(buf, '\0', total);
